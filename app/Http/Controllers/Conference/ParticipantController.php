...
class ParticipantController extends Controller
{
    ...
    public function store(ConferenceParticipantCreateRequest $request, $conferenceId)
    {
        ...
        if ($participantResult && $lectureResult) {
            $leadData = $data;
            $leadData['department_name'] = $participantResult->department->name;
            $leadData['conference_name'] = $participantResult->conference->name;
            $bx = new BitrixClient(config('services.bx24.token'), config('services.bx24.url'));
            $bx->addLead($leadData);
